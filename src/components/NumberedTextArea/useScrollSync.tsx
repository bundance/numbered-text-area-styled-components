...
export const useScrollSync = (textArea1: React.RefObject<HTMLTextAreaElement>, textArea2: React.RefObject<HTMLTextAreaElement>): void => {
  React.useEffect(() => {
    ...
    textArea1Instance?.addEventListener(SCROLL_EVENT, handleScroll(textArea1, textArea2), { capture: true });
    textArea2Instance?.addEventListener(SCROLL_EVENT, handleScroll(textArea1, textArea2), { capture: true });
